...
export default function DeviceHandoverPage() {
  ...
  useEffect(() => {
    if (!isLoading) {
      if (!user) {
        router.push('/login?returnUrl=/devices/handover');
        return;
      }
      ...
      if (!authorized) {
        console.warn('User not authorized for device handover:', user.role);
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-500 to-orange-600">
        <div className="text-center text-white max-w-md p-8">
          ...
          <p className="mb-4">
            You do not have permission to access the Device Handover module.
          </p>
          <p className="text-sm opacity-80 mb-4">
            Required roles: Production Lead, Production Staff, QC Lead, Admin, or Super Admin
          </p>
          <button

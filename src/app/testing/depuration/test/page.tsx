...
export default function DepurationTestPage() {
  ...
  useEffect(() => {
    if (!isLoading) {
      if (!user) {
        router.push('/login?returnUrl=/testing/depuration/test');
        return;
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-500 to-orange-600">
        <div className="text-center text-white max-w-md p-8">
          ...
          <p className="mb-4">You do not have permission to run depuration tests.</p>
          <button
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-blue-700 shadow-lg">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white">Run Depuration Test</h1>
              <p className="text-indigo-100 mt-1">Submit depuration test results</p>
            </div>
            <button
              onClick={() => router.push('/dashboard')}
